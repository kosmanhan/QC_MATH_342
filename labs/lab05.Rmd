...
#prediction: itself and zero
orthogonal_projection(c(1, 2, 3, 4), c(0, 2, 0, -1))
#prediction: O and itself
...
```{r}
newX=cbind(X, rnorm(n))
y_hat = newX%*%solve(t(newX) %*% newX)%*% t(newX) %*% y
...
As the number of columns is more than the rows of data, we no longer have an invertible matrix, and cannot get y_hat.
...
```
There are nine times as many projections as orthogonal ones. 

Is this ratio expected? Why or why not?

Yes, because when we individually orthogonally project vectors, we do not know if the projections are equivalent to previous ones. 
...
for (j in 2 : ncol(X)){
  ...
  for (k in 1:(j-1)){
    v_k = V[, k, drop = FALSE]
    V[, j] = V[, j] - orthogonal_projection(X[,j], v_k)$a_parallel
  }
}
```
...
They are equal. Sign does not make a difference to norm.
...
c(mean(RMSE_trains),mean(RMSE_tests))
```
...
Yes, as the number of features are increased, in-sample metrics get better, but OOS metrics, which are honest, get worse. 
...
oos_se_by_p_k= matrix(NA, nrow = min_ntrain, ncol = K)
...
```{r}
s_s_es_b_j = apply(oos_se_by_p_k, 1, sd)
cbind(
  ooss_e_by_p - 2 * s_s_es_by_j,
  ooss_e_by_p + 2 * s_s_es_by_j
)
```