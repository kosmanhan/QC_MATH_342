...
oos_s_e_no_missingness=sqrt(mean((y_hat_test - y_test)^2))
```
...
oos_s_e_w_missingness=sqrt(mean((y_hat_test - y_test)^2))
```
...
```{r}
oos_s_e_no_missingness-oos_s_e_w_missingness
```
...
oos_s_e_w_rf_missingness=sqrt(mean((y_hat_test - y_test)^2))
```

How much predictive performance was lost due to missingness when `missForest` imputation was used?

```{r}
oos_s_e_no_missingness-oos_s_e_w_rf_missingness
```

Why did `missForest` imputation perform better than naive imputation?

The missForest was able to better reflect the missing data, than the naive imputation which simply used the mean of the missing data.
...
```{r}
num_trees_values = c(1, 2, 5, 10, 20, 30, 40, 50, 100, 200, 300, 400) #add 500, 1000 later
...
for (m in 1 : length(num_trees_values)){
  ...
  oob_se_bagged_trees_mod_by_num_trees[m] = mod_bag
...
ggplot(data.frame(num_trees = num_trees_values, 
                  oob_se = oob_se_bagged_trees_mod_by_num_trees)) + 
...
wide_df = data.table(
  ...
  oob_se_bagged = oob_se_bagged_trees_mod_by_num_trees,
  oob_se_rf = oob_se_rf_mod_by_num_trees
)
long_df = melt(wide_df, measure.vars = c("oob_se_bagged", "oob_se_rf"))
ggplot(long_df)+ geom_line(aes(x = num_trees, y= value, color = variable))
```
...
```{r}
num_trees_values = c(1, 2, 5, 10, 20, 30, 40, 50, 100, 200, 300, 400) #do 500, 1000 at home
oob_se_bagged_trees_mod_by_num_trees = array(NA, length(num_trees_values))
for (m in 1 : length(num_trees_values)){
  mod_bag = YARFBAG(X_train, y_train, num_trees = num_trees_values[m])
  oob_se_bagged_trees_mod_by_num_trees[m] = mod_bag$misclassification_error
}
ggplot(data.frame(num_trees = num_trees_values, oob_se = 
