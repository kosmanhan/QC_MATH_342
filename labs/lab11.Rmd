...
y_test = adult[test_indices, ]$income
...
y_validation=as.numeric(y_validation)
g_b = qc_basement_gbm(data.frame(X_train_mm), y_train)
y_validation_hats_by_m = qc_basement_gbm_all_predictions(g_b, data.frame(X_validation_mm))
rmses_by_m = apply(y_validation_hats_by_m, 2, function(y_hat){sqrt(mean((y_validation - y_hat)^2))})
rmses_by_m
...
```{r}
#change rmse to missclassificiation
